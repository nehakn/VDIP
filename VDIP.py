...
class VDIP:
    ...
    def pb_red_band_clicked(self):
        ...
        logger.append('Red band location:')
        logger.append(str(red_band_path))
    ...
    def pb_nir_band_clicked(self):
        ...
        logger=self.dlg.tb_display
        logger.append('\nNIR band location:')
        logger.append(str(nir_band_path))
    ...
    def pb_compute_clicked(self):
        ...
        if self.dlg.cb_clip_bands.isChecked():
            ...
            logger=self.dlg.tb_display
            logger.append('\nChosen directory to store the clipped red and nir bands:')
            logger.append(str(r_n_raster_output))
            ...
            logger=self.dlg.tb_display
            logger.append('\nThe clipped red file path is:')
            logger.append(r_raster_output)
            ...
            logger=self.dlg.tb_display
            logger.append('\nThe clipped nir file path is:')
            logger.append(n_raster_output)
        ...
        # sinply get the red and nir bands in form of 2d array (using gdal for it)
            ...
            r_n_raster_output=str(QFileDialog.getExistingDirectory(self.dlg,"Select the output directory to save outputs"))
            logger=self.dlg.tb_display
            logger.append('\nDirectory selected to store the outputs:')
            logger.append(r_n_raster_output)
        ...
        if self.dlg.cb_local_soil_line.isChecked():
            ...
            logger=self.dlg.tb_display
            logger.append('\nSoil line slope:')
            logger.append(str(slope))
            logger.append('Soil line intercept:')
            logger.append(str(intercept))
        else:
            slope=1.0573
            intercept=0.0268
            logger=self.dlg.tb_display
